...
- まず用意済みの画像から、特殊行動カードをUnity上でカードとして表示・再生できるか確認する。
- 特殊行動カードは、通常の「展示・怪盗・檻」の基本効果に追加効果を持つ構成を基本とする。
- 最初は代表的な特殊カードだけを選び、ゲーム開始時に各プレイヤーへ2枚配布する。
- 行動カードの手札も、お宝手札と同じように横スクロールできるようにする。
- 特殊行動カードは使い切りとする。
- 特殊行動カードの取得方法は、ゲーム開始時の2枚と檻報酬時のみとする。
- 特殊行動カードの残数は脱落条件へ含めない。脱落判定は通常行動カードだけで行う。
- 各カードの追加効果は、使えるカードから1枚ずつ個別に実装・検証する。
