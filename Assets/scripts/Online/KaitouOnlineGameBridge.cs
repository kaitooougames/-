...
namespace KaitouOnline
{
    ...
    public sealed class KaitouOnlineGameBridge : MonoBehaviour
    {
        ...
        private void OnMessage(Envelope envelope)
        {
            if (envelope.type == MessageType.ActionRequest && session.IsHost)
            {
                ...
                if (request.action == "action_placement_ready")
                {
                    ...
                    int currentPlacementDay = HandManager.Instance != null
                        ? HandManager.Instance.CurrentDay : 1;
                    if (ready.day != currentPlacementDay) return;
                    ...
                    if (placementReadySeats.Count >= session.RoomPlayerCount)
                    {
                        session.Send(MessageType.RevealActions, -1,
                            Protocol.Json(new OnlineSeatChoice { day = currentPlacementDay }));
                    }
                    return;
                }
        ...
        private void ApplyReadyChoices(int snapshotDay, ActionCardChoice[] choices)
        {
            ...
            // SelectOnlineCardで通常の伏せ移動アニメーションは既に始まっている。
            // 68b6389の同期方式を使い、移動完了待ちや座標ワープは行わない。
            placementStarted = true;
            placementDay = snapshotDay;
            ReportPlacementStarted(snapshotDay);
        }

        private void ReportPlacementStarted(int day)
        {
