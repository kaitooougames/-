...
namespace KaitouOnline
{
    ...
    public sealed class KaitouOnlineGameBridge : MonoBehaviour
    {
        ...
        private void ApplyReadyChoices(int snapshotDay, ActionCardChoice[] choices)
        {
            ...
            revealStarted = true;
            WaitingMessage = "";
            StartCoroutine(RevealActionsAfterPlacement(snapshotDay));
            Debug.Log($"<color=#70E8FF>【オンライン】{snapshotDay}日目の選択が揃いました。</color>");
        }

        private System.Collections.IEnumerator RevealActionsAfterPlacement(int day)
        {
            yield return new WaitForSeconds(0.85f);
            int currentDay = HandManager.Instance != null
                ? HandManager.Instance.CurrentDay : 1;
            if (!revealStarted || day != currentDay) yield break;
